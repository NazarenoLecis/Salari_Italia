...
COVERAGE_ITEMS = [
    ...
    },
    {
        ...
        "note": "Le API pubbliche aggregate espongono primo decile, mediana, nono decile e media. L'export non ricostruisce una distribuzione piena senza classi o frequenze ufficiali.",
    },
    {
    ...
    },
    {
        ...
        "note": "I flussi settoriali RACLI integrati pubblicano il dettaglio Ateco per Italia, non per singola provincia. L'export non costruisce incroci provincia-settore senza celle ufficiali.",
    },
    {
...
def build_data_export_payload(
    ...
    payload = {
        "meta": {
            ...
            "description": "Export JSON riproducibile dei dati salariali armonizzati.",
            "methodology": (
    ...
    validate_data_export_payload(payload)
    return payload


def validate_data_export_payload(payload: dict[str, Any]) -> dict[str, Any]:
    records = payload.get("records")
    if not isinstance(records, list) or not records:
        raise ValueError("L'export dati non contiene record.")
    ...
    if not isinstance(schema, list) or "value" not in schema or "year" not in schema or "dataset" not in schema:
        raise ValueError("L'export dati non contiene uno schema record valido.")
    ...
    if invalid_records:
        raise ValueError(f"Record export non validi: {invalid_records[:5]}")
    filters = payload.get("filters")
    if not isinstance(filters, dict) or not filters:
        raise ValueError("L'export dati non contiene filtri.")
    coverage = payload.get("coverage")
    if not isinstance(coverage, list) or not coverage:
        raise ValueError("L'export dati non contiene la matrice di copertura.")
    return {
        "records": len(records),
